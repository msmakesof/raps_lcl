...
$listado=  mysql_query("SELECT grupo_noconformidades.*, estados.nombre_estados FROM grupo_noconformidades JOIN estados ON estados.cod_estados = grupo_noconformidades.estado ORDER BY nombre_corto",$cn);
    ...
    if ( msg ) {
		$.ajax({
			...
			success:function(respuesta){			
 				$('#fila_'+oId).empty();
				$('#borraitem').html('<div style="background-color:#CC0000;color:#FFFFFF;padding:12px;margin:12px;">Se ha eliminado correctamente el Grupo: '+nomemp+'.</div>').fadeOut(3000);
				$('#fila_'+oId).remove();
				location.reload();
 			}
		});					   
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="tabla_lista_paises">
    ...
    <tfoot>
      ...
      <tbody>
       ...
       {
			$codaudit = mb_convert_encoding($reg['id_grupo'], "UTF-8");
			$nombrecorto = $reg['nombre_corto'];
			$nombre = $reg['nombre_grupo'];
			echo '<tr id="fila_'.$codaudit.'">';
			echo '<td>';?>
			<?php if($row_rs_privilegios['control'] =='S'){ ?>
	  	<?php if($row_rs_privilegios['modifica'] =='Y'){ ?>
      <a href="#" onclick="TINY.box.show({iframe:'../grupos/upd.php?id=<?php echo $codaudit; ?>',boxid:'frameless',width:980,height:500,fixed:false,maskid:'blackmask',maskopacity:40,closejs:function(){closeJS()}})"  class="fuentetest3"><?php echo $codaudit; ?></a>
				<?php }
			}
			?>
			<?php
            ...
            <td>
						 <?php if($row_rs_privilegios['control'] =='S'){ ?>
								<?php if($row_rs_privilegios['graba'] =='Y'){ ?>										
										<a href="#" class="button white" id="a1" onclick="TINY.box.show({iframe:'../grupos/add.php',boxid:'frameless',width:998,height:540,fixed:false,maskid:'blackmask',maskopacity:40,closejs:function(){closeJS()}})">Adicionar</a>
								 <?php } 
								}								
								?>
						</td>
<!--            <td><a href="del_ciu" class="button white" id="a2">Borrar</a></td>-->
     ...
     $(document).ready(function(){
         $('#a1').tooltip ('Crear registros a la tabla Grupos.</p>', { width: 200, style: 'alert' });
         $('#a2').tooltip ('Borrar registros de la tabla Grupos.', { mode: 'tr', width: 200 });
		     $('#a3').tooltip ('Salir de la opci&oacute;n de Grupos.', { mode: 'tr', width: 200 });
