...
$listado=  mysql_query("SELECT DISTINCT listachqxtipoaudit.nombre_lc, tipo_auditoria.nom_ti_audit, auditoria.nombre_auditoria, listachqxtipoaudit.id_listacheq, listachqxtipoaudit.objetivo, listachqxtipoaudit.tipo_auditoria, listachqxtipoaudit.id_auditoria, lista_chequeo.proceso, procesos.proceson as nomproceso FROM listachqxtipoaudit JOIN tipo_auditoria ON tipo_auditoria.cod_ti_audit = listachqxtipoaudit.tipo_auditoria JOIN auditoria ON auditoria.id = listachqxtipoaudit.id_auditoria JOIN lista_chequeo ON lista_chequeo.tipo_auditoria = listachqxtipoaudit.tipo_auditoria AND lista_chequeo.id_auditoria = listachqxtipoaudit.id_auditoria AND lista_chequeo.id_listachq = listachqxtipoaudit.id_listacheq JOIN procesos ON procesos.abr2 = lista_chequeo.proceso AND procesos.estado ='A' WHERE listachqxtipoaudit.idproceso > 0 ORDER BY tipo_auditoria.nom_ti_audit, auditoria.nombre_auditoria, listachqxtipoaudit.nombre_lc, nomproceso ASC",$cn);
...
function borra_ta(oId, par2){
	var service = oId;
	var nomemp = par2;
    var msg = confirm("Desea eliminar el nombre de la Lista de Chequeo: "+ nomemp +" ?")
    if ( msg ) {
		$.ajax({
			type: "GET",
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="tabla_lista_paises">
    ...
    <tfoot>
      ...
      <a href="#" onclick="TINY.box.show({iframe:'../cronos/upd_lc.php?ta=<?php echo $v_tipo_audit; ?>&au=<?php echo $v_audit; ?>&lc=<?php echo $v_lc; ?>',boxid:'frameless',width:998,height:890,fixed:false,maskid:'blackmask',maskopacity:40,closejs:function(){closeJS()}})"  class="fuentetest3"><?php echo $i; ?></a>
				<?php }
			}
			echo '</td>';
			echo '<td>'.$nombre.'</td>';
			...
			echo '<td>';
			?>	
			<a href="../cronos/detallado.php?idta=<?php echo $v_tipo_audit; ?>&idaudi=<?php echo $v_audit; ?>&idlc=<?php echo $v_lc; ?>&proc=<?php echo $proc;?>" class="fuentetest3"><img src="../imagesaacion.jpg" width="25" height="25" border="0" align="absmiddle" />Ver Lista Detallada...</a>
			<?php
			echo '</td>';
			}
			echo '<td>';						
            ...
            <td>
						 <?php if($row_rs_privilegios['control'] =='S'){ ?>
								<?php if($row_rs_privilegios['graba'] =='Y'){ ?>										
										<a href="#" class="button white" id="a1" onclick="TINY.box.show({iframe:'../cronos/add.php',boxid:'frameless',width:1010,height:870,fixed:false,maskid:'blackmask',maskopacity:40,closejs:function(){closeJS()}})">Adicionar</a>
								 <?php } 
								}								
								?>
						</td>
